...
let cors =
  process.env.NODE_ENV === "development"
    ? "http://localhost:8080/" // Use CORS proxy for localhost development
...
function SearchGoogle(link) {
  return new Promise(async (resolve, reject) => {
    ...
    let results = $("h3").parent("div").parent("div").parent("a"); // Get the results from the page
    if (results.length) {
      results.each((i, elem) => {
...
*/
export function CreateJsonInfo(link) {
  return Promise.race([
